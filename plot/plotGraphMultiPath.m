...
function plotGraphMultiN( ...
    ...
    % ytickformat('%.1f');
    % plot(ctrl_inputs.Time, ctrl_inputs.Data(1,:) + mass * g, Color=color, LineWidth=line_width, DisplayName=strcat('$Margin{=}',num2str(Margin),'$'));
    % lgd.AutoUpdate = 'off';
    ...
    % ytickformat('%.1f');
    % plot(ctrl_inputs.Time, angular_speeds, Color=color, LineWidth=line_width, DisplayName=strcat('$Margin{=}',num2str(Margin),'$'));
    % lgd.AutoUpdate = 'off';
